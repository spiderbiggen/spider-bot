...
lazy_static! {
    ...
    static ref SOURCES: Vec<AnimeSource> = vec![
        AnimeSource::new("[SubsPlease]", Some("1_2"), "^\\[.*?] (.*) - (\\d+)(?:\\.(\\d+))?(?:[vV](\\d+?))? \\((\\d+?p)\\) \\[.*?\\].mkv", None, vec!["(1080p)", "(720p)", "(540p)", "(480p)"]),
    ];
}
...
impl AnimeSource {
    fn new<K>(key: K, category: Option<K>, regex: K, filter: Option<K>, resolutions: Vec<K>) -> AnimeSource where K: Into<String> {
        AnimeSource {
            key: key.into(),
...
struct AnimeComponents(String, String, String, Option<i32>, Option<i32>, Option<i32>);

impl AnimeComponents {
    fn from_string<S>(inp: Option<S>, regex: &Regex) -> Option<AnimeComponents> where S: Into<String> {
        inp.and_then(|s| Some(s.into()))
            ...
            .and_then(|cap| {
                ...
                Some(AnimeComponents(cap[0].into(), cap[1].into(), resolution, episode, decimal, version))
            })
    }
}

fn to_anime(item: Item, regex: &Regex) -> Option<Anime> {
    let date = item.pub_date.as_ref().and_then(|str| DateTime::parse_from_rfc2822(str).ok())?;
    let link = item.link?;
    let comments: String = item.guid?.value;
    let id: Option<String> = item.extensions.get("nyaa")
        .and_then(|nyaa| nyaa.get("infoHash"))
        .and_then(|info_hash| info_hash.deref().get(0))
        .and_then(|extension| extension.value.clone());

    AnimeComponents::from_string(item.title, regex)
        .and_then(|AnimeComponents(file_name, title, resolution, episode, decimal, version)| {
            Some(Anime {
                id,
                episode,
                decimal,
...
mod tests {
    ...
    fn test_parse_anime_components_basic() {
        ...
        let expected = AnimeComponents("[_] Test Anime - 01 (1080p) [_].mkv".into(), "Test Anime".into(), "1080p".into(), Some(1), None, None);
    ...
    fn test_parse_anime_components_with_version_lower() {
        ...
        let expected = AnimeComponents(input.into(), "Test Anime".into(), "1080p".into(), Some(1), None, Some(1));
        let source = SOURCES.get(0).unwrap();
        let regex = Regex::new(&source.regex).unwrap();
        let result = AnimeComponents::from_string(Some(input), &regex);
        assert!(result.is_some());
    ...
    fn test_parse_anime_components_with_decimal() {
        ...
        let expected = AnimeComponents(input.into(), "Test Anime".into(), "1080p".into(), Some(1), Some(1), None);
        let source = SOURCES.get(0).unwrap();
        let regex = Regex::new(&source.regex).unwrap();
        let result = AnimeComponents::from_string(Some(input), &regex);
        assert!(result.is_some());
    ...
    fn test_parse_anime_components_with_decimal_and_version() {
        ...
        let expected = AnimeComponents(input.into(), "Test Anime".into(), "1080p".into(), Some(1), Some(1), Some(1));
        let source = SOURCES.get(0).unwrap();
        let regex = Regex::new(&source.regex).unwrap();
        let result = AnimeComponents::from_string(Some(input), &regex);
        assert!(result.is_some());
